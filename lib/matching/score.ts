...
export function calculateRelevanceScore(
  ...
  // Rule-based boosts
  if (job.locations && profile.preferred_locations?.some(loc => job.locations?.join(', ').toLowerCase().includes(loc.toLowerCase()))) {
    score += 0.05;
  }
  ...
  const overlappingSkills = job.skills?.filter(skill => profile.skills?.includes(skill)) || [];
...
export const buildJobText = (job: NormalizedJob): string => {
    const jobText = `${job.title} at ${job.company}. Location: ${job.locations?.join(', ')}. Description: ${job.description_text}. Skills: ${job.skills?.join(', ')}`;
